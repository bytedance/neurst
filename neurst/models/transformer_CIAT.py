...
@register_model(["CIAT"])
class TransformerCIAT(EncoderDecoderModel):
    ...
    def __init__(self, args, *largs, **kwargs):
        ...
        """
        super(TransformerCIAT, self).__init__(args, *largs, **kwargs)
        self._args = args

    @staticmethod
    def class_or_method_args():
        this_args = [x for x in super(TransformerCIAT, TransformerCIAT).class_or_method_args()
        ...
        this_args += [
            ...
            Flag("is_pretrain", dtype=Flag.TYPE.BOOLEAN, default=True,
                 help="Tell if it is in pretrain stage"),
            Flag("adapter.layer_adapter_dim", dtype=Flag.TYPE.INTEGER, default=None,
                 help="dim of adapter, default to half model dim"),
            Flag("adapter.emb_adapter_dim", dtype=Flag.TYPE.INTEGER, default=None,
                 help="embedding adapter dim, default to half model dim"),
    ...
    def new(cls, args, src_meta, trg_meta, name=None):
        ...
        for f in cls.class_or_method_args():
            if f.name in args:
                ...
                elif f.name.startswith("adapter."):
                    encoder_params[f.name[8:]] = args[f.name]
                    decoder_params[f.name[8:]] = args[f.name]
                elif f.name == "is_pretrain":
                    encoder_params["is_pretrain"] = args[f.name]
                    decoder_params["is_pretrain"] = args[f.name]
                    src_modality.trainable = args[f.name]
                    trg_modality.trainable = args[f.name]
        # build encoder and decoder
        encoder = build_encoder({
            "encoder.class": "TransformerEncoderCIAT",
            "encoder.params": encoder_params})
        decoder = build_decoder({
            "decoder.class": "TransformerDecoderCIAT",
